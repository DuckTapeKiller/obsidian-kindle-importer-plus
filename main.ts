        ...
        if (targetFile instanceof TFile) {
            try {
                ...
                if (this.settings.addTags && this.settings.tagsToAdd) {
                    // Fix: Added await to processFrontMatter
...
class FilePickerModal extends Modal {
    ...
    onOpen() {
        ...
        new Setting(contentEl).setName("Import Highlights").setHeading();
...
class KindleHighlightsSettingsTab extends PluginSettingTab {
    ...
    display(): void {
            ...
            if (this.plugin.settings.includeFrontmatter) {
                new Setting(containerEl)
                    ...
                    .addTextArea((text) => {
                        ...
                        // Fix: Avoid setting style directly on element property
                        text.inputEl.setCssStyles({ width: "100%", height: "200px" });
                    });
            }
        }

        // --- Tagging ---
        new Setting(containerEl).setName("Tag Injection").setHeading();
        ...
        // --- Note Configuration ---
        new Setting(containerEl).setName("Note Customization").setHeading();
        ...
        // --- Colors ---
        new Setting(containerEl).setName("Callout Mappings").setHeading();
